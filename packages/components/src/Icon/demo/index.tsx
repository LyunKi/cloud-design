...
import { FlexLayout, View, Button, Icon } from '@cloud-design/components/src'
...
export default function IconList() {
  ...
  const [switchMode, setSwitchMode] = useState('outline')
  return (
    <SafeAreaView>
      <FlexLayout direction="column">
        <View ts={{ marginBottom: 24 }}>
          <Button
            ts={{ width: 160 }}
            onPress={(_event) =>
              setSwitchMode((prev) => (prev === 'outline' ? 'fill' : 'outline'))
            }
            value={`Switch to ${switchMode}`}
          />
        </View>
        <FlexLayout style={{ flexWrap: 'wrap' }}>
          {icons.map((item) => {
            const iconName = switchMode === 'outline' ? item : `${item}-outline`
            return (
              <View
                key={item}
                style={{
